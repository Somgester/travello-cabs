      ...
      {open ? (
        <div className='md:hidden z-50 fixed'>
          <div className='ox-2 pt-2 pb-3 space-y-1'>
            {NavLinks.map((link) => (
              <a href={link.link} key={link.title} className='text-black text-md font-appleui z-50 block py-2 rounded-md font-medium bg-black'>{link.title}</a>
            ))}
          </div>
        </div>
      ) : null}
